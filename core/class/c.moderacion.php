...
class tsMod {
   ...
   public function getContenido() {
      ...
      # MURO
      $data['muro'] = result_array(db_exec([__FILE__, __LINE__], 'query', 'SELECT m.pub_id, m.p_user, m.p_user_pub, m.p_ip, m.p_date, m.p_body, u.user_id, u.user_name FROM u_muro AS m LEFT JOIN u_miembros AS u ON m.p_user_pub = u.user_id WHERE '.($tipo === 1 ? 'm.p_ip' : 'm.p_body').' '.$met));
      # USUARIOS
      $data['usuarios'] = result_array(db_exec([__FILE__, __LINE__], 'query', 'SELECT user_id, user_name, user_last_ip, user_lastlogin, user_lastactive FROM u_miembros WHERE '.($tipo == 1 ? 'user_last_ip' : 'user_name').' '.$met.' ORDER BY user_lastactive DESC'));
      # POSTS
      $data['posts'] = result_array(db_exec([__FILE__, __LINE__], 'query', 'SELECT p.post_id, p.post_user, p.post_title, p.post_date, p.post_ip, u.user_name, c.c_nombre, c.c_seo, c.c_img FROM p_posts AS p LEFT JOIN u_miembros AS u ON p.post_user = u.user_id LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE ' .($tipo == 1 ? 'p.post_ip ' . $met . '' : 'p.post_title ' . $met . ' OR p.post_body '.$met)));
      # FOTOS
      $data['fotos'] = result_array(db_exec([__FILE__, __LINE__], 'query', 'SELECT f.foto_id, f.f_title, f.f_user, f.f_date, f.f_ip, u.user_name FROM f_fotos AS f LEFT JOIN u_miembros AS u ON f.f_user = u.user_id WHERE ' .($tipo == 1 ? 'f.f_ip ' . $met . '' : 'f.f_title ' . $met .' OR f.f_description ' . $met)));
      # POSTS COMENTARIOS
      $data['p_comentarios'] = result_array(db_exec([__FILE__, __LINE__], 'query', 'SELECT u.user_id, u.user_name, c.* FROM p_comentarios AS c LEFT JOIN u_miembros AS u ON u.user_id = c.c_user WHERE ' .($tipo == 1 ? 'c.c_ip ' . $met . '' : 'c.c_user ' . $met . ' OR c.c_body ' . $met)));
      # FOTOS COMENTARIOS
      $data['f_comentarios'] = result_array(db_exec([__FILE__, __LINE__], 'query', 'SELECT u.user_id, u.user_name, f.* , c.* FROM f_comentarios AS c LEFT JOIN u_miembros AS u ON u.user_id = c.c_user LEFT JOIN f_fotos AS f ON f.foto_id = c.c_foto_id WHERE ' .($tipo == 1 ? 'c.c_ip ' . $met . '' : 'c.c_user ' . $met . ' OR c.c_body ' . $met)));
      # COUNTS
   ...
   */
   public function deletePost($pid) {
      global $tsCore, $tsNotificaciones, $tsUser;
      $pid = intval($pid);
      if ($tsUser->is_admod || $tsUser->permisos['moep']) {
         ...
         if (db_exec([__FILE__, __LINE__], 'query', 'UPDATE `p_posts` SET `post_status` = 2 WHERE `post_id` = '.$pid)) {
            ...
            if ($_POST['send_b'] == 'yes') {
               ...
               $status = $tsNotificaciones->setAviso($data['post_user'], 'Post eliminado', $aviso, 1);
   ...
   public function deleteMps($mid) {
      ...
      if ($tsUser->is_admod || $tsUser->permisos['moadm']) {
         ...
         if ($query = db_exec([__FILE__, __LINE__], 'query', 'SELECT m.mp_from, m.mp_subject, u.user_name FROM u_mensajes AS m LEFT JOIN u_miembros AS u ON m.mp_from = u.user_id WHERE m.mp_id = \'' . (int)$mid . '\' LIMIT 1')) {
            ...
            $status = $tsNotificaciones->setAviso($data['mp_from'], 'Mensaje eliminado', $aviso, 1);
            // ELIMINAR DENUNCIAS
      ...
      if ($tsUser->is_admod || $tsUser->permisos['moadf'] || $tsUser->permisos['moef']) {
         ...
         if (db_exec([__FILE__, __LINE__], 'query', 'UPDATE `f_fotos` SET `f_status` = 2 WHERE `foto_id` = ' . $fid)) {
            ...
            if ($data['f_user'] != $tsUser->uid) {
               ...
               $status = $tsNotificaciones->setAviso($data['f_user'], 'Foto eliminada', $aviso, 1);
            }
            // ELIMINAR DENUNCIAS
   ...
   public function setSticky($post_id) {
      ...
      if ($tsUser->is_admod || $tsUser->permisos['most']) {
         ...
         if ($data['post_sticky'] == 1) {
            db_exec([__FILE__, __LINE__], 'query', 'UPDATE `p_posts` SET `post_sticky` = \'0\' WHERE `post_id` = \'' . (int) $post_id . '\'');
            return '1: El post fue quitado de la home.';
         } else {
            db_exec([__FILE__, __LINE__], 'query', 'UPDATE `p_posts` SET `post_sticky` = \'1\' WHERE `post_id` = \'' . (int) $post_id . '\'');
   ...
   public function setOpenClosed($post_id) {
      ...
      if ($tsUser->is_admod || $tsUser->permisos['moayca']) {
         ...
         if ($data['post_block_comments'] == 1) {
            return (db_exec([__FILE__, __LINE__], 'query', 'UPDATE `p_posts` SET `post_block_comments` = 0 WHERE `post_id` = '.$post_id)) ? '1: El post fue abierto.' : '0: Hubo un error al abrir el post';
         } else {
            return (db_exec([__FILE__, __LINE__], 'query', 'UPDATE `p_posts` SET `post_block_comments` = 1 WHERE `post_id` = '.$post_id)) ? '1: El post fue cerrado.' : '0: Hubo un error al cerrar el post';
         }
   ...
   */
   public function setHistory($action, $type, $data) {
      global $tsUser, $tsNotificaciones, $tsCore;
      //
      if ($type == 'post') {
         switch ($action) {
            case 'borrar':
            ...
            case 'editar':
               ...
               $tsNotificaciones->setAviso($data['autor'], 'Post editado', $aviso, 2);
               $_SERVER['REMOTE_ADDR'] = $_SERVER['X_FORWARDED_FOR'] ? $_SERVER['X_FORWARDED_FOR'] :$_SERVER['REMOTE_ADDR'];
         ...
         // DATOS
         $foto = db_exec('fetch_assoc', db_exec([__FILE__, __LINE__], 'query', 'SELECT `foto_id`, `f_description`, `f_title`, `f_user` FROM `f_fotos` WHERE `foto_id` = \'' . (int)$data . '\' LIMIT 1'));
         switch ($action) {
            case 'borrar':
               // RAZON
               $razon = $tsCore->setSecure($_POST['razon']);
