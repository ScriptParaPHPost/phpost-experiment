...
class tsMuro {
    ...
    */
    public function streamPost(){
        ...
        // MONITOR
        $tsNotificaciones->setNotificacion(12, $pid, $tsUser->uid, $pub_id);
        // ACTIVIDAD
       ...
       $pub = db_exec('fetch_assoc', $query);
        ...
        if($pub['p_user'] > 0){
            ...
            if(db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO `u_muro_comentarios` (`pub_id`, `c_user`, `c_date`, `c_body`, `c_ip`) VALUES (\''.(int)$pid.'\', \''.$tsUser->uid.'\', \''.$date.'\', \''.$tsCore->setSecure($data, true).'\', \''.$_SERVER['REMOTE_ADDR'].'\')')){
                ...
                // MONITOR
                $tsNotificaciones->setMuroRepost($pid, $pub['p_user'], $pub['p_user_pub']);
                // ACTIVIDAD
    ...
    */
   function likePost(){
        global $tsUser, $tsCore, $tsNotificaciones, $tsActividad;
        // ANTI FLOOD
        ...
        if(empty($i_like)){
			if(db_exec([__FILE__, __LINE__], 'query', 'INSERT INTO u_muro_likes (user_id, obj_id, obj_type) VALUES (\''.$tsUser->uid.'\', \''.(int)$id.'\', \''.(int)$type.'\')')){
                ...
                // MONITOR
                $tsNotificaciones->setNotificacion(14, $exists['uid'], $tsUser->uid, $id, $type);
                // ACTIVIDAD
