    ...
    switch($action){
        ...
        case 'avisos':
            ...
            if(empty($_GET['aid']) && empty($_GET['did'])){
                $smarty->assign("tsMensajes",$tsNotificaciones->getAvisos());
            } elseif($_GET['aid']) {
                $smarty->assign("tsMensaje",$tsNotificaciones->readAviso($_GET['aid']));
            } elseif($_GET['did']){
                $borrado = $tsNotificaciones->delAviso($_GET['did']);
